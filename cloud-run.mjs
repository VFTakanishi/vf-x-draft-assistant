...
function buildDrafts(date) {
  ...
  return {
    morning: `朝のひとこと。${pick(morningTopics, seed)}で差が出るのは、知識量より普段の感覚です。『まだ走れるし』で流す前に、昨日までと違うかだけ見ておくとトラブルはかなり減ります。`,
    noon: `昼のメモ。${pick(noonTopics, seed + 7)}で迷ったら、まず慌てて自己判断しすぎないことが大事です。安全を優先して、症状を一つずつ整理するだけでも判断ミスは減らせます。`,
    evening: `最近よく思うのは、${pick(eveningTopics, seed + 13)}ということです。大きな故障になる前の『ちょっと気になる』の段階で相談してもらえるほうが、現場としてはずっと助かります。`
  };
}
